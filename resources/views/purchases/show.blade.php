<x-marketplace-layout title="Detail Pembelian | Jepret" description="Detail order Jepret." :noindex="true">
            ...
            <div class="border-x border-b border-public-line p-5 sm:p-8"><div class="divide-y divide-public-line">
                @forelse($transactions as $transaction)
                    <article class="grid gap-4 py-5 first:pt-0 last:pb-0 sm:grid-cols-[112px_minmax(0,1fr)_auto] sm:items-center">
                        ...
                        <div><h2 class="text-sm font-extrabold text-public-ink">{{ $transaction->photo?->title ?: ($transaction->photo?->event?->nama_event ?? 'Foto Jepret') }}</h2><p class="mt-1 text-xs font-bold text-public-muted">{{ $transaction->photo?->fotografer?->name ?? 'Photographer' }}</p><p class="mt-2 text-sm font-extrabold text-public-ink">Rp{{ number_format($transaction->total_bayar, 0, ',', '.') }}</p></div>
                ...
                @empty
                    <div class="py-10 text-center text-sm font-semibold text-public-muted">Tidak ada foto dalam pembelian ini.</div>
                @endforelse
            </div></div>
        </div>
    </section>
