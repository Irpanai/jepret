                ...
                <div class="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="text" value="{{ Auth::user()->name ?? 'Dwi Cahya' }}" readonly class="w-full bg-gray-50 border border-gray-200 text-gray-500 text-sm rounded-xl px-4 py-3 focus:ring-0 cursor-not-allowed">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Email</label>
                            <input type="email" value="{{ Auth::user()->email ?? 'dwi@gmail.com' }}" readonly class="w-full bg-gray-50 border border-gray-200 text-gray-500 text-sm rounded-xl px-4 py-3 focus:ring-0 cursor-not-allowed">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1">Nomor WhatsApp <span class="text-gray-400 font-normal">(Opsional)</span></label>
                            <input type="text" placeholder="Contoh: 08123456789" class="w-full bg-white border border-gray-300 text-gray-900 text-sm rounded-xl px-4 py-3 focus:ring-black focus:border-black transition">
                            <p class="text-xs text-gray-500 mt-1">Kami mungkin akan mengirimkan link download via WhatsApp.</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm sticky top-24">
                    ...
                    <div class="space-y-4 mb-6">
                        ...
                        <div class="flex justify-between items-start gap-4">
                            <div>
                                <h4 class="text-sm font-bold text-gray-900 leading-tight">{{ $item['event'] ?? 'Foto' }}</h4>
                                <p class="text-xs text-gray-500">{{ $item['fotografer'] ?? 'Photographer' }}</p>
                            </div>
                            <div class="text-sm font-bold text-black shrink-0">Rp {{ number_format($item['price'], 0, ',', '.') }}</div>
                        </div>
                        @endforeach
                    </div>
